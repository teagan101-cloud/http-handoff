...
HTTP Handoff's developer is Teagan Doherty. The plugin reads the request files and local text body files you select to create a collection on your computer. It does not transmit those files to the developer, collect conversion telemetry, execute the source requests or read private environment files. The exported file is under your control. Importing or running it in another application is subject to that application's behavior and terms.

JetBrains operates Marketplace accounts, orders, payments and license activation under its own privacy notice. The IDE supplies the plugin's license status. The developer may receive limited order and license information from JetBrains to administer the product and support customers.
...
The developer will update this notice if the product's data handling changes. No new collection or telemetry is enabled by this notice alone.
